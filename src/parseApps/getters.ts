...
export const appPath = memoize(
  function _appPath(appsPath: string, type?: APP_TYPES, section?: string, name?: string): string {
    let path = appsPath
    if (type) {
      path = join(path, type)
    }
  ...
  (appsPath: string, type: keyof typeof APP_TYPES | '' = '', section = '', name = '') =>
    `${appsPath}/${type}/${section}/${name}`,
)

export const getApps = memoize(
  async function _getApps(appsPath: string, type?: APP_TYPES): Promise<appDesc[]> {
...
export const getCordovaAnalysisFiles = memoize(
  async function _getCordovaAnalysisFiles(
    appsPath: string,
    app: appDesc,
    ...
    const nonFlatLocationId = await resolveAllOrInParallel(
      locations
        .filter((location) => !location.startsWith('_'))
        .map((location) => async () => {
...
export const getReactNativeAnalysisFiles = memoize(
  async function _getReactNativeAnalysisFiles(
    appsPath: string,
    app: appDesc,
    ...
    return ids
      .filter((id) => reactNativeIdRegex.test(id))
      .map((path) => {
...
export const getAnalysisFiles = memoize(
  async function _getAnalysisFiles(appsPath: string, app: appDesc): Promise<analysisFile[]> {
